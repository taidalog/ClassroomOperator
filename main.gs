function createCourses() {
  ...
  if (firstCell.getValue() === "" || values.length === 1) {
    Browser.msgBox("新しく作成するクラスの情報を入力してから実行してください。\\nA列の name のみ必須です。", Browser.Buttons.OK);
    return false;
  }
  ...
  requests.
    map(request => {
      ...
      const courseState = request.courseState !== "" ? request.courseState : "PROVISIONED";
...
function resetCourseCreationSheet() {
  ...
  const res = Browser.msgBox("現在のシートの内容を消去します。\\n実行しますか？", Browser.Buttons.OK_CANCEL);
  if (res === "cancel") {
    return false;
  }
  
  const headers = ["name", "section", "descriptionHeading", "description", "room", "ownerId", "courseState"];
  const referenceUrl = "https://developers.google.com/classroom/reference/rest/v1/courses#Course.FIELDS-table"
...
function listCourses() {
  ...
  const courseProperties = myCourses.courses.map(course => properties.map(property => course[property]));
  ...
  sh.getRange(2, 2, courseProperties.length, courseProperties[0].length).setValues(courseProperties);
...
function removeCourses() {
  const res = Browser.msgBox("クラスを一括削除します。\\n実行しますか？", Browser.Buttons.OK_CANCEL);
  if (res === "cancel") {
    return false;
  }
...
function invokeArchiveOrRemovecourses(action) {
  ...
  const values = firstCell.getDataRegion().getValues();
  if (firstCell.getValue() === "" || values.length === 1) {
    Browser.msgBox("まず初めにクラスの一覧を作成し、それから実行してください。", Browser.Buttons.OK);
    return false;
  }
  ...
  const actionName = action === "ARCHIVE" ? "アーカイブ" : action === "REMOVE" ? "削除" : "";
...
function createInvitations() {
  ...
  const values = firstCell.getDataRegion().getValues();
  if (firstCell.getValue() === "" || values.length === 1) {
    Browser.msgBox("以下の通りに情報を入力してから実行してください。A列以外は全て必須です。\\nA列: クラス名\\nB列: クラス ID\\nC列: ユーザー ID\\nD列: 役割 (STUDENT/TEACHER)", Browser.Buttons.OK);
    return false;
  }
  ...
  requests.
    map(request => {
      return { courseId: String(request.courseId), userId: request.userId, role: request.role };
    }).
...
function resetInvitationSheet() {
  ...
  const referenceUrl = "https://developers.google.com/classroom/reference/rest/v1/invitations#Invitation.FIELDS-table"
  const sh = SpreadsheetApp.getActiveSheet();
...
function validateRequest(request) {
  ...
  return {
    ...
    matched: (course.succeed && course.course.name === request.name && course.course.id == request.id)
  };
}
...
function newObjectsFrom2DArray(arr) {
  ...
  return records.map(record =>
    Object.fromEntries(record.map((value, i) =>
      [headers[i], value])));
}